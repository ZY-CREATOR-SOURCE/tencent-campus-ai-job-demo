...
const API_BASE_URL = "https://tencent-campus-ai-job-demo.onrender.com";
...
function normalizeProfileJob(job, index) {
  const category = job.category || "核心推荐";
  const tags = Array.isArray(job.matched_keywords) && job.matched_keywords.length
    ? job.matched_keywords
    : ["画像匹配", "AI推荐"];

  return {
    tier: tierByCategory[category] || "core",
    title: job.title || `AI推荐岗位${index + 1}`,
    company: `${job.company || "腾讯"} · ${job.department || "校招岗位"}`,
    city: job.city || "深圳",
    salary: job.salary || "面议",
    score: Number(job.match) || 82,
    reason: job.reason || job.description || "该岗位与你的求职画像匹配度较高。",
    tags: tags.slice(0, 5),
    gapTitle: "基于求职画像生成：岗位关键词匹配建议",
    gapText: `该岗位与${tags.slice(0, 3).join("、") || "你的核心经历"}较匹配，建议在简历中进一步强化相关关键词和量化结果。`,
    bars: [86, 84, 82],
    gaps: tags.slice(0, 4),
    before: "负责相关项目或实习工作，参与数据分析和项目推进。",
    after: "围绕目标岗位要求，突出项目目标、个人行动、使用工具和量化结果，将经历改写为更贴近腾讯校招初筛标准的表达。"
  };
}

function renderCareerProfile(apiData) {
  const careerProfile = apiData?.profile || {};
  const recommendedJobs = Array.isArray(careerProfile.recommended_jobs)
    ? careerProfile.recommended_jobs
    : [];

  hasParsedResume = true;
  els.profileSummary.textContent = careerProfile.profile_summary || "已生成求职画像。建议围绕学历背景、硬技能、实习经历和软实力继续优化简历表达。";
  renderTags(els.educationTags, careerProfile.education || ["已识别教育背景"]);
  renderTags(els.hardSkills, careerProfile.hard_skills || ["数据分析基础", "办公软件应用"]);
  renderTags(els.experienceTags, careerProfile.internship_experience || ["项目/实习经历待进一步提炼"]);
  renderTags(els.softSkills, careerProfile.soft_skills || ["沟通协作", "执行推进", "学习能力"]);

  if (recommendedJobs.length) {
    jobs.splice(0, jobs.length, ...recommendedJobs.map(normalizeProfileJob));
    activeTier = jobs[0]?.tier || "core";
  ...
  renderJobs();
  els.statusPill.textContent = careerProfile.mode === "openai" ? "AI画像已生成" : "画像已生成 · 本地规则兜底";
}

async function generateCareerProfile(file) {
  if (!file) {
  ...
  els.statusPill.textContent = "正在生成求职画像...";
  els.fileStatus.textContent = `正在解析：${file.name}`;
  els.uploadDrop.classList.add("dragging");
  els.startBtn.disabled = true;
  els.startBtn.textContent = "生成中...";
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(data.detail || "求职画像生成失败");
    }
    renderCareerProfile(data);
    els.fileStatus.textContent = `已生成画像：${file.name}`;
    els.workspace.scrollIntoView({ behavior: "smooth", block: "start" });
  } catch (error) {
    els.statusPill.textContent = "画像生成失败";
    els.fileStatus.textContent = error.message || "请确认后端服务已部署成功";
  } finally {
    els.uploadDrop.classList.remove("dragging");
    els.startBtn.disabled = false;
    els.startBtn.textContent = "生成求职画像";
  }
}
...
function handleResumeFile(file) {
  ...
  els.fileStatus.textContent = `已选择：${file.name}，点击“生成求职画像”开始解析`;
}
...
els.startBtn.addEventListener("click", () => {
  ...
  generateCareerProfile(selectedResumeFile);
});
